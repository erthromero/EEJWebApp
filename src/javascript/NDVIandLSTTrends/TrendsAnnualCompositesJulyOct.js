...
// statistical trends providing green space classification results over Bay Area Cities
// This code takes median composites at 1-year intervals for the months of July to October
...
var cloudMask457_C2 = function(image) {
  ...
  // NOTE (Eric): Function that applies scaling factors Applies scaling factors.
  ...
  // NOTE (Eric): Filter from July 1st to October 1st and add
  // the collection year as a property to each image
  .filter(ee.Filter.dayOfYear(182, 274))
  .map(addYearProp);
  ...
  print('1 year final list', finalList);
  ...
  //print(collection_ndvi);
  //Map.addLayer(collection_ndvi.filterBounds(ext_geom).select('constant'));
  ...
  Export.image.toDrive({image: NDVILinFit, description: 'NDVILinearFit1yr1990to2020JulyOct',
                  folder: "EEJSpatialTrendRasters", scale: 30 , region: ext_geom ,
                  maxPixels:1e13});
  
  // NOTE (Eric): Export LST Trends to Google Drive
  
  Export.image.toDrive({image: LSTLinFit, description: 'LSTLinearFit1yr1990to2020JulyOct',
  ...
  Export.image.toDrive({image: collection_ndvi, description: 'NDVI1yrTimeSeries1990to2020JulyOct',
                  folder: "EEJSpatialTrendRasters", scale: 30 , region: ext_geom ,
                  maxPixels:1e13});
  
  // NOTE (Eric): Export LST image collection as multi-band single raster
  
  Export.image.toDrive({image: collection_lst, description: 'LST1yrTimeSeries1990to2020JulyOct',
                  folder: "EEJSpatialTrendRasters", scale: 120 , region: ext_geom ,
                  maxPixels:1e13});
